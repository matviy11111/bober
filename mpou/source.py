...
class Snake():
    ...
    def eatApple(self):
        ...
        if self.snakeX[0] == apple.getAppleX() and self.snakeY[0] == apple.getAppleY():
            ...
            x = self.snakeX[len(self.snakeX)-6] # Snake grows
            y = self.snakeY[len(self.snakeY) - 3]
            self.snakeX.append(x+3)
    ...
    def repaint(self):
        ...
        if snake.checkGameOver() == False:
            ...
            canvas.create_rectangle(snake.getSnakeX(0) * tilesize, snake.getSnakeY(0) * tilesize,
                                    ...
                                    snake.getSnakeY(0) * tilesize + tilesize, fill="cyan")
            ...
            for i in range(1, snake.getSnakeLength(), 1):
                canvas.create_rectangle(snake.getSnakeX(i) * tilesize, snake.getSnakeY(i) * tilesize,
                                        ...
                                        snake.getSnakeY(i) * tilesize + tilesize, fill="white")
            canvas.create_rectangle(apple.getAppleX() * tilesize, apple.getAppleY() * tilesize,
                                    apple.getAppleX() * tilesize + tilesize,
                                    apple.getAppleY() * tilesize + tilesize, fill="purple")

        else:
            canvas.delete(ALL)
            canvas.create_text(800, 400, fill="red", font="Times 20 italic bold", text="GameOver!")
            canvas.create_text(800, 150, fill="white", font="Times 20 italic bold",
